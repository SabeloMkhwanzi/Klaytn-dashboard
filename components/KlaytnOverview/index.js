...
import KlaytnStats from "./KlaytnStats";
import { IconX } from "@tabler/icons";
//import LoaderComp from "../LoaderComp";

export default function KlaytnOverview() {
  // used React-Query to fetch Covalent API
  const { data, error, isFetching } = useQuery(["ecosystem"], async () => {
    const res = await fetch(
      "https://api.coingecko.com/api/v3/coins/klay-token/market_chart?vs_currency=usd&days=183&interval=daily"
    );
    return res.json();
  });
